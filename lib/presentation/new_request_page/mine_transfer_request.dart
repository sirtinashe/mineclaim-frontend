...
import 'package:mineclaim/presentation/new_request_page/widgets/transfer_request.dart';
...
class MineTransferRequest extends StatefulWidget {
  MineTransferRequest({Key? key}) : super(key: key);

  @override
  State<MineTransferRequest> createState() => _MineTransferRequestState();
}

class _MineTransferRequestState extends State<MineTransferRequest> {
    ...
    return Container(
      child: StreamBuilder<QuerySnapshot>(
        ...
        builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
          ...
          List<TransferRequests> serializedRequests =  RequestSerializer.serializeTransferRequests(documents);
          return ListView.builder(
            itemCount: serializedRequests.length,
            itemBuilder: (BuildContext context, int index) {
              // DocumentSnapshot document = documents[index];
              TransferRequests transferRequests = serializedRequests[index];
              // print(document);

              return Card(
                child: Padding(
                  padding: EdgeInsets.only(bottom: 10.0.v),
                  child: TransferRequestWidget(
